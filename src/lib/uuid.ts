/**
 * A v4 UUID that survives an insecure context.
 *
 ...
 *
 * `Math.random()` is the last resort only. Real entropy is free wherever the
 * API exists, so we only degrade when it does not.
 *
 * Extracted from sessionId.ts once a second caller appeared: the learner id
 * (learnerId.ts) needs exactly the same guarantee for exactly the same
 * reason, and the LAN origin that motivated this is where device testing
 * happens — so a duplicate would be a second place to get it wrong.
 */
