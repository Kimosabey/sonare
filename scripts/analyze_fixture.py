...
def main():
    ...
    header = f"{'platform':<18}{'set':<5}{'n':>4}{'p25':>8}{'median':>8}{'p75':>8}"
    print(header)
    print("-" * len(header))
    for platform in sorted(by_platform):
        for set_id in sorted(by_platform[platform]):
            values = by_platform[platform][set_id]
            print(
                f"{platform:<18}{set_id:<5}{len(values):>4}"
    ...
    # ── separation ───────────────────────────────────────────────────────────
    print(f"{'platform':<18}{'separation':>12}   verdict")
    print("-" * 52)

    overall_rows = []
    for platform in sorted(by_platform):
        a = by_platform[platform].get("A", [])
        b = by_platform[platform].get("B", [])
        if not a or not b:
            print(f"{platform:<18}{'—':>12}   needs both sets on this platform")
            continue
        sep = percentile(a, 0.25) - percentile(b, 0.75)
        verdict = "separates" if sep > 0 else "DOES NOT separate"
        print(f"{platform:<18}{sep:>12.1f}   {verdict}")
        overall_rows.append((platform, sep))

    all_a = [r["accuracy"] for r in scored if r["set"] == "A"]
    all_b = [r["accuracy"] for r in scored if r["set"] == "B"]
    if all_a and all_b:
        sep = percentile(all_a, 0.25) - percentile(all_b, 0.75)
        print("-" * 52)
        print(f"{'ALL':<18}{sep:>12.1f}   {'separates' if sep > 0 else 'DOES NOT separate'}")
        print()
        if sep <= 0:
...
def plot(by_platform):
    ...
    fig, axes = plt.subplots(1, len(platforms), figsize=(4 * len(platforms), 4.5), squeeze=False)

    for ax, platform in zip(axes[0], platforms):
        sets = sorted(by_platform[platform])
        data = [by_platform[platform][s] for s in sets]
    ...
    fig.suptitle("Set A (accented, correct) vs Set B (deliberately wrong)")
    fig.tight_layout()
